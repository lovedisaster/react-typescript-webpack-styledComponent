...
const Step1 = (props: Step1PropsType) => {
  ...
  useEffect(() => {
    ...
    setPlans(
      plansClone.map((planItem: PlanCategoryType) => {
  ...
  return (
    <Step1Wrapper>
      ...
      <ButtonsWrapper data-testid="test-plans-section">
        {plans.map((planItem: PlanCategoryType) => (
          <CategoryButton
            data-testid={planItem.index}
            key={planItem.index}
            onClick={() => {
              props.dispatch({
                type: Actions.SELECT_PLAN_CATEGORY,
                payload: { selected: planItem.index, selectedValue: planItem.value.toLowerCase() },
              });
            }}
